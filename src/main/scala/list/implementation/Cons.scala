...
case class Cons(head: Int, tail: IntList) extends SinglyLinkedIntList {
  ...
  override def append(elem: Int): IntList = isEmpty match {
    case true => Cons(elem, Empty)
    case false => Cons(head, tail.append(elem))
  }

  override def contains(elem: Int): Boolean =
    if(elem == head) true else tail.contains(elem)
